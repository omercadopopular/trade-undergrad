...
\title[]{International Trade: Lecture 15}
...
\begin{frame}{Demand}
    \begin{wideitemize}
        ...
        \item<2-> $\Phi_i := \{1,2,\cdots, N\}$ is the set of all goods available in the domestic economy
        \item<3-> They want to consume all goods available, but want low prices

        \item<4-> How do they decide how to substitute across goods depending on the price?

        \item<5-> Elasticity of substitution:
...
\begin{frame}{Demand}
    \begin{wideitemize}
        \item Before, we worked we Cobb-Douglas preferences:
        \begin{equation*}
            U_i(Q_C,Q_R) = Q_C^{\alpha} Q_R^{1-\alpha}, \qquad 0 < \alpha < 1
        \end{equation*}

        \item<2-> Cobb-Douglas is a special case in which the elasticity of substitution $\sigma_{CR} = 1$

        \item<3-> What does that mean?
        \begin{itemize}
            \item Suppose relative price of computers increases $\uparrow$ by 1\% 
            \item Then consumers shift relative quantities of computers purchased down $\downarrow$ by 1\% 
            \item Quantities adjust inversely to prices at a one to one ratio 
            \item Implication: expenditure shares are constant at $\frac{P_C Q_{i,C}}{I_i} = \alpha$ 
        \end{itemize}

        \item<4-> Is this realistic in many cases?

        \item<5-> Can you substitute away from gasoline?
        (yes, but difficult on the short run)

        \item<6-> Much easier to substitute Lay's for Ruffles.
...
\begin{wideitemize}
            ...
            \item<2-> Key parameter:  $\sigma > 1$ is the elasticity of substitution (same for every good)
            
            \item<3-> Larger $\sigma$ is:
            \begin{itemize}
                \item high $\sigma$: more responsive to changes in prices (think different brands of bottled water) 
                \item low $\sigma$: less responsive to changes in prices (think steak vs tofu)
            \end{itemize}
...
\begin{wideitemize}
        ...
        \begin{eqnarray*}
            &\max_{\{q_i(\varphi)\}_{\
        \varphi \in \Phi}}& Q_i \equiv \left[ q_i(1)^{\tfrac{\sigma-1}{\sigma}} + \cdots +q_i(
        N)^{\tfrac{\sigma-1}{\sigma}} \right]^{\tfrac{\sigma}{\sigma-1} } \\
        &s.t.& \qquad  P_i Q_i = p_i(1) q_i(1) + \cdots + p_i(N) q_i(N)\le I_i = w_i L_i 
        \end{eqnarray*}
        \textcolor{gray}{\scriptsize (don't get scared with the math, I want to you get the intuition)} \\
\blue{Notation}:
        \begin{wideitemize}
            \item<2-> $q_i(\varphi)$: quantity demanded of good $\varphi$
            \item<2-> $p_i(\varphi)$: price of good $\varphi$
            \item<3-> Sum over the elements of set $\Phi_i:=\{1, 2, \cdots, N\}$ \\
            ...
            \item<5-> Total expenditure across all goods = cost of the consumption basket  $P_i Q_i$
        \end{wideitemize}
...
\begin{wideitemize}
    ...
    \item<3-> To produce a given quantity $q_i(\varphi)$, labor used is:
    ...
    \item<4-> Only labor $\ell > \bar{f}$ contributes to extra production
...
\begin{wideitemize}
    \item Monopolists incorporate into their problem the fact that their choice of prices changes demand (or equivalently, that their choice of production changes prices).
    \item<2-> They take demand functions as given and pick price to maximize profits, maximizing:
    \begin{equation*}
        \max_{p} \pi_i = (p-MC)q(p) - w_i \bar{f}
    \end{equation*}

    \item<3-> \blue{Solution} (using product rule):
    \begin{eqnarray*}
        p \times q'(p) +  q(p) - MC \times q'(p) &=& 0 \\
        p \times q'(p) +  q(p) &=& MC \times q'(p) \\
        p  +  \frac{q(p)}{q'(p)} &=& MC
    \end{eqnarray*}
    
    \item<4-> Therefore: \begin{equation*}
