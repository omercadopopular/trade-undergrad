...
cloth sector relative to tech.
    ...
    \item Cloth sector uses little capital relative to labor (the slope is very flat) while the tech sector is, as expected, relatively more capital intensive (the slope is steeper)
\end{wideitemize}
...
\begin{frame}{Rybczynski Theorem}
        ...
        \begin{wideitemize}
            ...
            \item<2-> Note that since relative goods prices $P_C/P_T$ are assumed to be fixed, $w_i/r_i$ also fixed \\
...
\begin{equation*}
    \left. \frac{\partial  \ell_{i,C}}{\partial \bar{K}_i} \right|_{w_i / r_i} \textcolor{gray}{= - \frac{ 
...
\begin{frame}{Empirical evidence (i)}
    \begin{figure}
        \centering
        \includegraphics[width=0.6\linewidth]{figs/textbook-ho-1.jpg}
    \end{figure}
\end{frame}
...
\begin{frame}{Empirical evidence (ii)}
    \begin{figure}
        \centering
        \includegraphics[width=0.58\linewidth]{figs/textbook-ho-2.jpg}
    \end{figure}
\end{frame}

\begin{frame}{Conclusion and discussion}
\begin{wideitemize}
    \item In HO, $P_C/P_T$ uniquely determine $w/r$ when both goods are produced.
    \item Higher price for the labor-intensive good raises real wages and lowers real returns to capital.
    \item Expanding one factor increases output of good using it intensively (contracts other sector).
    \item Countries export goods that intensively use their abundant factors (H-O theorem).
    \item Trade alters income distribution: abundant-factor owners gain, scarce-factor owners lose, though total welfare can rise.
    
    \item Factor-price equalization holds in theory but not in practice due to resource, technology, and trade barriers.
    \item Modified H-O models explain trade patterns and factor content reasonably well.
