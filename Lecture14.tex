...
\title[]{International Trade: Lecture 14}
...
\begin{wideitemize}
    ...
    \onslide<5->{
    \item When $\bar{f}>0$ and $C(Y) > \bar{f}$, $AC(Y)$ is decreasing in Y $\implies$ increasing returns to scale!
    }
\end{wideitemize}
...
\begin{frame}{Logic of a monopolist}
  ...
  \begin{wideitemize}
      \item Now suppose monopolists are large enough they now their supply influences prices
      \item In other words, they know prices $P(Y)$ are some function of output
      \begin{equation*}
        \max_{\{Y\}} \pi = P(Y) Y - C(Y) 
    \end{equation*}


      \item<2-> Optimal choice:

      \begin{equation*}
          MR = \underbrace{P(Y) + P'(Y)Y}_{\text{by product rule}} = C'(Y) = MC
      \end{equation*}

      \item<3-> To sell incremental unit, firm must lower price for all units 

      \item<4-> e.g., suppose $P(Y) = (A/b) - (1/b)Y$, then $P'(Y)Y =  - (1/b)Y$ and:

      \begin{equation*}
          MR = P(Y) + P'(Y)Y= (A/b) - (1/b)Y - (1/b)Y = (A/b) - (2/b)Y = MC 
      \end{equation*}
...
\begin{wideitemize}
    ...
    \item<2-> Oligopoly (duopoly): Several (two) firms serve the market and compete partially, each anticipating each others’ rational actions
    \item<3-> Monopolistic competition: No reaction to other firms’ supply
    \begin{itemize}
